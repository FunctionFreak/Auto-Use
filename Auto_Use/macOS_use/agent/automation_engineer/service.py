...
class AgentService:
    """QA automation-engineer agent service.

    Runs the iterative test loop for the QA agent: it drives a macOS app/website
    purely through GUI interactions and reports pass/fail with evidence. Structurally
    identical to the main_driver AgentService (same loop, same context-optimization /
    memory cleanup, same response schema) but it auto-loads the QA system_prompt.md
    that sits beside this file and is exercised through the click/GUI-only tool set.
    Selected in place of main_driver when QA mode is enabled.
    """
