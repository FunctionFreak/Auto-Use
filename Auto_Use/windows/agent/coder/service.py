...
from .view import (CLIAgentResponseFormatter, render_tool_response, snapshot_turn,
                   decode_step, encode_step, decode_results, encode_results,
                   wire_calls_from, compression_dump, compression_entry)
...
class AgentService:
    ...
    def __init__(self, provider: str, model: str, save_conversation: bool = False,
        ...
        if self.save_conversation:
            ...
            # Minions run as SUBPROCESSES spawned by the controller, which
            # copies this process's environment — so exporting the flag here is
            # what makes each dispatched minion write its own snapshots to
            # cli_minion_conversation/{its own sid}/, without the spawn command
            # (controller-owned) needing to change. Debugging the coder without
            # its scouts' transcripts is half a picture.
            os.environ["AUTOUSE_SAVE_CONVERSATION"] = "1"
    ...
    def _save_conversation_snapshot(self, messages: list, current_assistant_response: str, interaction_count: int):
        ...
        turns, which are rendered through snapshot_turn so each past step
