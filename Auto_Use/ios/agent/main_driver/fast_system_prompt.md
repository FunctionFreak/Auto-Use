...
6. Only the FIRST call of the turn carries `memory`; every other call passes "".
7. Example - all three targets visible on the current screen, so all three calls go in ONE turn (two taps, then type):
[{"type": "click", "id": 44}, {"type": "click", "id": 18}, {"type": "input", "id": 4, "value": "conjuring"}]
