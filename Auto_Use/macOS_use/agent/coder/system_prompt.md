...
2. Run it and read the ACTUAL output — exit code, stdout, stderr. Proof = the real passing output, not "it should work". If it fails, fix the code and re-run until it genuinely passes.
...
4. Once proof is in hand: record a one-line result in `scratchpad` (e.g. "Verified: parser handles empty input — 6/6 cases pass"), then DELETE the residue — `rm -rf ./.autouse_verify/` plus any other throwaway check files you made. Keep ONLY your real changes and any tests the user explicitly asked for; leave the workspace clean.
