...
# ═════════════════════════════════════════════════════════════════════════════
# Tool-output rendering — the coder loop runs every routed action_result
# through render_tool_response(). It lives HERE in the agent's own formatter
# module (macOS keeps the equivalent in controller/cli/service.py; on Windows
# the controller is off-limits, so the renderer sits next to the other output
# formatting instead). The envelope:
#
#   <Tool_response>
...
_CLIP_FOOTERS = {
    ...
    "shell": "... [long output clipped — re-run with filters (| Select-Object -First N, | Select-Object -Last N, | Select-String) or write to a file and grep/view it]",
}
