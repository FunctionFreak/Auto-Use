...
- A provider with no native web-search API is acceptable only when the platform's `web` tool routes the query to the browser agent (`Auto_Use/web`) on the same provider+model and returns its report — see `<platform>/controller/tool/web/web_agent.py` (identical on mac/windows/ios; Together AI is the first such provider).
