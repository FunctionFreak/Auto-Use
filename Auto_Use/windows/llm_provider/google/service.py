...
class GoogleProvider:
    ...
    def __init__(self, api_key: str = None, cli_agent: bool = False, model: str = None, vertex_project_id: str = None, vertex_location: str = None, tools: list = None):
        ...
        # Native tool calling: Gemini function declarations — the model's
        # output contract. None only for mode="text" (plain prose).
        # additionalProperties is stripped (Gemini rejects it).
        # _clean_schema_for_google rebuilds dicts rather than mutating, so
    ...
    def send_request(self, messages: list, model: str, annotated_screenshot_base64: Optional[str] = None) -> Dict[str, Any]:
        ...
        # shape; Gemini carries the same information as function_call /
        # function_response PARTS. Gemini has no call ids — it matches a
        # response to its call BY NAME — so remember each call's name against
        # the id the loop keyed its result to.
        call_names = {}
        ...
        if self.tools:
            ...
            # mode="ANY" - every turn must call at least one tool; a text-only
        ...
        try:
            ...
            if self.tools:
                # response.text is None once any part carries a function call,
                # so walk the parts directly: collect prose and every
                # function_call as {"name", "arguments": dict}. fc.args is
                # already a mapping — no JSON decode needed.
                texts = []
                calls = []
