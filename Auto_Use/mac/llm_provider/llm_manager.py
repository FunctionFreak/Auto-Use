...
_MAIN_TRACK_PARAMS = {
    ...
    "memory": 'Follow the <memory> rules - line 1 is the verdict on the previous step\'s guard, judged on the CURRENT screenshot: "S<n> ok" or "S<n> fail: <short why>" ("S1 start" on the first step). Then key context (app/screen state, tool name + result) and, for any step touching UI, the "Targets: id N (name/type/...)" line resolved from the CURRENT element_tree. 2-4 concise lines. Fill on the FIRST tool call of the step; pass "" on every additional call in the same step.',
...
_MAIN_TRACK_PARAMS_FAST = {
    "memory": 'Your ONLY reasoning field: the verdict on what the last action did to the screen ("S<n> ok" / "S<n> fail: <why>"), plus the context the next step needs (app/screen state, resolved [id]s, key values). Keep it tight. Fill on the FIRST tool call of the step; pass "" on every additional call in the same step.',
}
...
- If a result returns `error: permission_dialog`, a macOS permission popup is blocking the command and couldn't be auto-clicked. Do NOT blindly retry — report it to the user and ask them to grant Auto Use **Accessibility + Full Disk Access** (System Settings > Privacy & Security), then retry once they confirm.
...
- Examples:
  ...
  4. Pair pattern — grep first, then view a narrow range:
     ...
     (grep returns `Auto_Use/mac/agent/cli/service.py:233: ...`)
     Step 2: "action": [{"type": "view", "path": "Auto_Use/mac/agent/cli/service.py", "start": 220, "end": 260}]"""),
...
- Examples:
  ...
  3. Two facts confirmed in one step — two separate calls:
     "action": [{"type": "scratchpad", "value": "**Verified:** parser handles empty input — **6/6** cases pass"}, {"type": "scratchpad", "value": "**Saved:** report exported to `/Users/me/Desktop/q3_report.pdf`"}]"""),
...
- Good examples (state what you want, not how to get it):
  ...
  2. "action": [{"type": "minion", "value": "list all imports of ScratchpadService under Auto_Use/mac/ with line numbers + direct usages."}]
...
MINION_TOOLS = [
    _tool("shell", {"command": {"type": "string"},
                    "input": {"type": "string"}},
          r"""Native zsh/bash - **READ-ONLY commands only** (e.g. `ls`, `find`, `file`, `stat`, `wc -l`, `head`, `tail` without redirection, `which`). Never run anything that writes, deletes, moves, or otherwise mutates state. Always include `input: ""`.
...
- Allowed examples:
  ...
  3. "action": [{"type": "shell", "command": "wc -l Auto_Use/mac/agent/service.py", "input": ""}]
...
- Format: "action": [{"type": "scratchpad", "value": "<one-line verified note in markdown format>"}]
- Examples:
  1. "action": [{"type": "scratchpad", "value": "`Auto_Use/mac/agent/service.py:254` — `_read_scratchpad_from_file` definition"}]
  2. "action": [{"type": "scratchpad", "value": "`Auto_Use/mac/controller/view.py:620` — `action_type == \"scratchpad\"` routing branch"}]
...
def _main_tools(track: dict) -> list:
    ...
    twice: quality mode with three tracking params, fast mode with `memory`
    alone.

    macOS-specific vs the Windows registry: `input`/`typewrite` carry `value`
    (not `text`), and `applescript` (app + value) and `drag_drop` exist here
    only. `clicks` stays on right_click/screenshot even though the controller
    ignores it, because the old strict union required it."""
    return [
        # -- <Tool_Capability> ------------------------------------------------
        _tool("open_app", {"value": {"type": "string"}},
              'Launch an installed application (.app only). No manual search required within the OS.\n'
        ...
        _tool("input", {"id": {"type": "integer"}, "value": {"type": "string"}},
              ...
              '    3. Example: {"type": "input", "id": 9, "value": "hi, how are you"}', track=track),
        _tool("typewrite", {"value": {"type": "string"}},
              ...
              '    2. Example: {"type": "typewrite", "value": "hi, how are you"}', track=track),
        ...
        _tool("hotkey", {"value": {"type": "string"}},
              ...
              '        2. {"type": "hotkey", "value": "cmd+shift+s"}', track=track),
        ...
        _tool("drag_drop", {"value": {"type": "string"}},
              'click-hold on one element and release on another (drag and drop).\n'
              '    1. Format: {"type": "drag_drop", "value": "<from_id> to <to_id>"}\n'
              '    2. Example: {"type": "drag_drop", "value": "8 to 15"}', track=track),
...
class LLMManager:
    ...
    def _initialize_provider(self):
        ...
        if self.provider == "openrouter":
            ...
            from Auto_Use.mac.llm_provider.google.view import get_model_info as get_google_info
            ...
            if is_vertex:
                ...
                try:
                    ...
                    # old walk stopped one level short at mac/api_key/,
                    # a folder that never existed, so Vertex config always read
                    # back empty here.
