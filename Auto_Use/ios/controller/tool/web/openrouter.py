...
from Auto_Use.ios.llm_provider import LLM_HTTP_TIMEOUT
...
def web_search(query, model, api_key=None):
    ...
    Args:
        query: Search query
        model: Full OpenRouter model name (e.g., "google/gemini-3.6-flash").
               Resolved by service.py through llm_provider/openrouter/view.py,
               so this path already follows that map and needs no model edit
               of its own.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("Example: python openrouter.py google/gemini-3.6-flash")
        sys.exit(1)
    model = sys.argv[1]
    query = input("Search: ")
