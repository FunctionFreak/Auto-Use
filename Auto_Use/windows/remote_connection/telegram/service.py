...
Setup:
  ...
  4. python -m Auto_Use.windows.remote_connection.telegram.service
...
# uninstalling AutoUse no longer wipes the bot token along with every API key.
...
# uses ([mac/remote_connection/telegram/service.py:78-85]).
PROVIDER_KEY_MAP = {
...
def _get_models_for_provider(provider_id: str) -> list:
    """Read MODEL_MAPPINGS from Auto_Use/windows/llm_provider/<id>/view.py
    and return non-hidden entries as [{id, display_name}, …]."""
    try:
        mod = importlib.import_module(
            f"Auto_Use.windows.llm_provider.{provider_id}.view"
        )
...
def _run_agent(task, provider, model, chat_id, bot, loop, prior_history=None,
    ...
    # actual task.
    from Auto_Use.windows.remote_connection.banner import StatusBanner, CoderBannerManager
