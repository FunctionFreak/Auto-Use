...
def _cleanup_scratchpad():
    """Clear all contents inside Auto_Use/mac/scratchpad/ for a fresh start."""
...
class AgentService:
    ...
    def __init__(self, provider: str, model: str, save_conversation: bool = False, frontend_callback=None, text_callback=None, web_callback=None, shell_callback=None, cli_callback=None, tool_callback=None, token_callback=None, api_key: str = None, stop_event=None, external_terminal: bool = False, prior_history: Optional[dict] = None, speed: str = "quality"):
        ...
        # Speed mode: "fast" swaps in fast_system_prompt.md and trims the output
        # schema to memory/next_goal/action.
        ...
        # and `next_goal`, leaving `memory` alone).
    ...
    def _skills_block(domain_block: str) -> str:
        ...
        carries no empty tag. NOTE: the matched .md already opens with its own
        <additional_knowledge>/<domain_knowledge> tag, which is why this wrapper
        is named <skills> - naming it the same would self-nest."""
    ...
    def _trim_history_entry(self, entry: str) -> str:
        ...
        LEGACY ONLY: native turns are append-only and never rewritten (rewriting
        them would invalidate the prompt-cache prefix on every request), so this
        now runs solely on schema-era entries from resumed chats.
        """
    ...
    def process_request(self, task: str) -> dict:
        ...
        json_fail_count = 0  # Track consecutive turns that produced no usable tool call (max 3)
        ...
        while True:
            ...
            if cli_await_result or pending_web_response:
                ...
                # Already encoded at its final size/format by the scanner
                # (LLM_IMAGE_* in tree/element.py) - re-compressing here would
                # only add a decode/encode round trip and destroy the labels.
                ...
                # Skills are matched against the screen we JUST scanned - so they
                # are looked up here and nowhere else. A digest iteration never
                # reaches this branch, so it can no longer match skills against
                # the previous screen's app name and an empty element tree.
                if not uac_detected:
            ...
            # Handle UAC secure desktop detection. Dead code on macOS: the
            # scanner hard-returns uac_detected=False (tree/element.py), so this
            # branch never runs - it is kept for parity with the Windows driver.
            if uac_detected:
                print("UAC detected - asking agent to accept or decline")
                image_sent = False
                formatted_element_tree = ""
                user_message = """<UAC_Trigger>
A UAC prompt is blocking the screen. Based on your previous actions, do you want to allow this?
...
{formatted_element_tree}"""
            ...
            # Build the API messages as a real NATIVE dialogue:
            #   system -> opening user turn (the objective) -> for each past step:
            #   assistant(prose + its tool_calls) -> one role:"tool" result per
            #   call -> finally the live user message (screen + state).
            #
            # APPEND-ONLY: nothing already in the transcript is ever rewritten, so
            # the prompt-cache prefix stays valid across steps. The old per-step
            # "trim the action out of older steps" pass is gone - it invalidated
            # that prefix on every single request.
            #
            # EVERY step replays its results, including the most recent: a saved
            # tool call with no matching result is a malformed transcript that
            # every provider rejects. That is also why <last_response> is gone -
            # the result now arrives in its canonical place, keyed to its call.
            #
            ...
            # Live user message (screen + persistent memory + skills + element
            # tree). Rebuilt every step and never persisted; the screenshot
            # attaches to it. The system prompt is added exactly once (above) -
            # the resumable seed never carries a system prompt, so a continued
            # session can't double it.
            ...
            try:
                ...
                # Save raw response before any parsing (for debugging). Under
                # native tool calling this is a dict, not a string.
                    ...
                    if agent_response:
                        ...
                        if "action" in agent_response and agent_response["action"]:
                            ...
                            if web_results_list:
                                ...
                                print("Web results captured for digest iteration")
                            else:
                                pending_web_response = None
                ...
                print(error_msg)
                # Same pairing guarantee for a failure ANYWHERE in the step. The
                # NameError guard covers an exception raised before the closers
                # were defined (e.g. the LLM call itself failing).
                try:
                    _close_pairing(f"error: {str(e)}")
                except NameError:
                    pass
