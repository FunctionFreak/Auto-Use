...
6. Example - all four targets visible on the current screen, so all four calls go in ONE turn (two clicks, then type, then submit):[{"type": "left_click","id": 44,"clicks": 1}, {"type": "left_click", "id": 18, "clicks": 1}, {"type": "input", "id": 4, "text": "iphone"}, {"type": "hotkey", "value": "enter"}].
7. Mixed example (tools + UI, one turn): `update_todo` value "1" + `input` id 19 text "www.google.com" + `hotkey` value "enter" + `scratchpad` value "Done: search submitted".
</efficiency_guideline>
<task_completion>
