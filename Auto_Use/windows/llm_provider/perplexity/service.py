...
class PerplexityProvider:
    ...
    def __init__(self, api_key: str, cli_agent: bool = False, model_info: dict = None, tools: list = None):
        ...
        # Native tool calling: Responses-style flat function tools — the
        # model's output contract. None only for mode="text" (plain prose).
    ...
    def send_request(self, messages: list, model: str, annotated_screenshot_base64: Optional[str] = None) -> Dict[str, Any]:
        ...
        for msg in messages:
            if msg["role"] == "system":
                ...
                # NATIVE TRANSCRIPT translation. Responses carries a tool
                # result as its own FLAT output item, not a role-message.
                ...
                # Only emit the text item when there IS text: under forced
                # tool_choice an assistant turn is often calls-only, and an
                # empty output_text block is rejected.
                if text.strip():
                ...
                for tc in (msg.get("tool_calls") or []):
                    ...
                    input_messages.append({
                        ...
                        "name": fn.get("name") or "",
                        # Outbound arguments must be a JSON STRING here.
        ...
        if self.tools:
            ...
            # tool). Canonical rationale: see openrouter/service.py. If the
            # Responses validator ever rejects this value, flip to "auto" —
            # the no-tool-called nudge in the loop is the backstop.
        ...
        try:
            ...
            # Normalize to choices[0].message.content format. NO early break:
            # the function_call items sit AFTER the message item, so the walk
            # must run to the end of `output` to see them.
            text_content = ""
            calls = []
