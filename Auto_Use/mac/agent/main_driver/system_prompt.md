...
5. Examples:
  1. "memory": "S4 ok. Gmail compose window open as predicted. Targets: id 12 (name='To', type='Edit', active='True')."
  2. "memory": "S6 fail: still on Home after Downloads click - toolbar target didn't register. Finder front. Targets: id 18 (name='Downloads', type='Button', sidebar)."
</memory>
<next_goal>
...
6. Examples:
  ...
  2. "next_goal": "Doing: recover the failed click - open Downloads via the sidebar (ToDo: Locate abc.pdf). If Finder shows the Downloads folder contents, then Next: double-click abc.pdf in the file list."
  3. "next_goal": "Doing: open Spotify and wait 3s (ToDo: Play focus playlist). If the Spotify main window is visible, then Next: think: survey the surface and route to the playlist."
</next_goal>
<action>
...
5. Example - a batched turn as you emit it (3 calls):
   call 1: update_todo {"thinking": "OBSERVE: ... VERIFY: ... PROGRESS: ... PLAN: ... PREDICT: ...", "memory": "S7 ok. Finder shows report_q1.pdf, report_q2.pdf, report_q3.pdf in ~/Desktop/reports - tasks 3-5 verified.", "next_goal": "Doing: mark tasks 3-5 complete (ToDo: task_5). If <todo_list> shows tasks 3-5 as [x], then Next: call done with the summary.", "value": "3"}
...
6. Example - UI batch on a skip step, all targets on the current screen (click, type, submit):
   call 1: left_click {"thinking": "not required", "memory": "S6 ok. Safari front, empty window. Targets: id 44 (name='Address and Search', type='TextField', active='True').", "next_goal": "Doing: open google.com via the address bar (ToDo: Search iPhone). If the Google home page with its search box is visible, then Next: type 'iphone' into the Google search box.", "id": 44, "clicks": 1}
   call 2: input {"thinking": "", "memory": "", "next_goal": "", "id": 44, "value": "www.google.com"}
