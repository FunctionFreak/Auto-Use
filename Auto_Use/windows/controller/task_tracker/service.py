...
class TaskTrackerService:
    def __init__(self, cli_mode: bool = False, session_id: str = None):
        ...
        """
        # Walk up two levels (controller/task_tracker -> windows) to locate the package root
        current_dir = os.path.dirname(os.path.abspath(__file__))
        windows_dir = os.path.dirname(os.path.dirname(current_dir))

        if cli_mode:
            if session_id:
                self.todo_dir = os.path.join(windows_dir, "scratchpad", "cli_todo", session_id)
            else:
                self.todo_dir = os.path.join(windows_dir, "scratchpad", "cli_todo")
        else:
            self.todo_dir = os.path.join(windows_dir, "scratchpad", "todo")
