...
class PlanService:
    ...
    def __init__(self, cli_mode: bool = False, session_id: str = None):
        ...
        # Go up two levels from Auto_Use/windows_use/controller/tool/ to reach windows_use/
        windows_use_dir = os.path.dirname(os.path.dirname(current_dir))

        if cli_mode:
            if session_id:
                self.plan_dir = os.path.join(windows_use_dir, "scratchpad", "cli_plan", session_id)
            else:
                self.plan_dir = os.path.join(windows_use_dir, "scratchpad", "cli_plan")
        else:
            self.plan_dir = os.path.join(windows_use_dir, "scratchpad", "plan")
