...
# Main Agent Output Schema (with vision support) — iPhone action set
# Uses anyOf discriminated union — each action type carries only its own fields (no nulls, no waste)
# Grouped by field signature to minimize anyOf branches (3 instead of 13)
AGENT_OUTPUT_SCHEMA = {
    "name": "agent_response",
    "strict": True,
    "schema": {
                ...
                "items": {
                    "anyOf": [
                        ...
                        },
                        {
                            "type": "object",
                            "properties": {
                                "type": {"type": "string", "enum": ["input", "scroll", "vault"]},
                                "id": {"type": "integer"},
                                "value": {"type": "string"}
                            },
                        ...
                        },
                        {
                            "type": "object",
                            "properties": {
                                "type": {"type": "string", "enum": ["open_app", "wait", "web", "shell", "todo_list", "update_todo", "video_player", "scratchpad", "done"]},
                                "value": {"type": "string"}
                            },
                            "required": ["type", "value"],
                            "additionalProperties": False
        ...
        "required": ["thinking", "eval", "decision", "next_goal", "memory", "action"],
        "additionalProperties": False
    }
}
