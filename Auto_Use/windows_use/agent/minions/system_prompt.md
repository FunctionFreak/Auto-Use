...
1. MAP — get your bearings before deep-diving (cheap, high-leverage).
   - If the target area is unfamiliar, first see the shape of it: `glob` the relevant tree or `shell tree /f` / `Get-ChildItem` for directory layout; `grep --files_with_matches` to see WHICH files mention the topic before reading any. This scopes the search so you don't grep the whole repo blindly.
...
Examples:
- "memory": "S3 ok. grep found _read_scratchpad at service.py:249 (definition confirmed). Expect: view 240-270 shows the function body + return."
