...
4. Judge success/failure of the last action using <os_vision> as primary ground truth (not <last_response>), comparing the screen against the predicted change stored in the previous step's "memory".
  1. Example: you might have `"action": [{"type": "input", "id": 74, "value": "abc@gmail.com"}]` with a success response in <last_response>, even though inputting text actually failed. If the expected change is missing on screen, treat it as FAIL: recovery becomes the "Now" step in `next_goal`, and note the failure in one short clause in this step's `memory`.
...
# Rules:
1. If the last action FAILED verification (rule 4), open with one short clause naming the failure (e.g., "tap on id 18 did not register; retrying via tab bar"). Skip this entirely when it passed.
2. Record what matters next: current app/screen state, key ids used, and any tool outputs.
3. For each interacted element, store: id + (element_name/type/value) from <element_tree>.
4. If a tool was used, store: tool name + query/purpose + the important result.
5. End with the predicted visible change of THIS step's action (screen/field value/state) prefixed "Expect:", so the next step can verify against the new screenshot.
6. Keep 2–4 concise lines total.
7. Examples:
  1. "memory": "video_player streaming check returned 'Streaming'; Disney+ playback confirmed. Next: pause via video_player. Expect: next streaming check reports paused."
  2. "memory": "Typed 'Netflix' into id 33 (element_name='Search', type='searchfield'); clicked id 37 (element_name='Netflix', type='icon'). Expect: Netflix app page open with Update/Open button visible."
</memory>
<next_goal>
...
5. Examples:
  ...
  2. "next_goal": "Now: recover the failed input — enter 'abc@gmail.com' into id 53 (ToDo: Enter login email). Plan: fill password via vault -> tap Sign In. Then: verify the field shows the email."
</next_goal>
<action>
