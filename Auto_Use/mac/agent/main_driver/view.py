...
# ---------------------------------------------------------------------------
# NATIVE TRANSCRIPT CODEC - main-driver flavor (mirrors the coder's codec in
# agent/coder/view.py, with two deltas: THREE tracking params {thinking,
# memory, next_goal}, and legacy tool slots re-wrapped in <tool_response> so
...
def snapshot_turn(text: str, calls: list) -> str:
    ...
    fields. The API payload is untouched: the model keeps receiving the native
    transcript."""
    actions = []
    ...
    return json.dumps(
        {"thinking": thinking or (text or "").strip() or "not required",
         "memory": memory, "next_goal": next_goal, "action": actions},
