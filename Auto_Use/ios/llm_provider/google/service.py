...
def _function_response_part(name: str, output: str, call_id: str = ""):
    ...
    (several scratchpad writes, a click plus an input). Falls back to the
    name-only part for ids we synthesized ourselves and for older SDKs."""
    if call_id:
        try:
...
class GoogleProvider:
    ...
    def __init__(self, api_key: str = None, cli_agent: bool = False, model: str = None, vertex_project_id: str = None, vertex_location: str = None, tools: list = None):
        ...
        # non-mutating, so the shared MAIN_TOOLS registry is safe.
    ...
    def send_request(self, messages: list, model: str, annotated_screenshot_base64: Optional[str] = None) -> Dict[str, Any]:
        ...
        # NATIVE TRANSCRIPT translation. The driver speaks the canonical OpenAI
        ...
        if self.tools:
            ...
            # `done` tool). Canonical rationale: see openrouter/service.py.
