...
3. Combine multiple calls in ONE turn when it speeds things up safely - they execute in the order you emit them, so emit them in the order they must run.
  1. Example - a batched turn as you emit it (3 calls):
   call 1: update_todo {"memory": "File Explorer shows report_q1.pdf, report_q2.pdf, report_q3.pdf in D:\\Drive\\reports - tasks 3-5 verified. Now: mark tasks 3-5 complete (ToDo: task_5). Plan: call done. Then: call done with the summary. Expect: <todo_list> shows tasks 3-5 as [x].", "value": "3"}
   call 2: update_todo {"memory": "", "value": "4"}
   call 3: update_todo {"memory": "", "value": "5"}
  2. Example - UI batch, all targets on the current screen (click, type, submit):
   call 1: left_click {"memory": "Chrome front, empty window; address bar id 44 (name='Address and search bar', type='Edit', active='True'). Now: open google.com via the address bar (ToDo: Search iPhone). Plan: type 'iphone' -> submit. Then: type 'iphone' into the Google search box. Expect: Google home page with its search box visible.", "id": 44, "clicks": 1}
   call 2: input {"memory": "", "id": 44, "text": "www.google.com"}
